...
void Read_DMP(void)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        Angle_root = atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * 57.3; // 原始角度
        error_Angle = Angle_root - last_Angle;                                                     // 与上一个角度的差值
        last_Angle = Angle_root;                                                                   // 保存当前角度作为上一个角度

        if (error_Angle > +180)
            Angle_Over -= 360; // 设定设备极限转速低于120转/分钟 每5毫秒最大转角低于3.6° =120/60/200 *360
        if (error_Angle < -180)
            Angle_Over += 360;         // 则理论上若出现前后差值绝对值大于3.6，小于359-3.6=355.4的，都可以视为是经过临界点，但是为了更加严谨，将差值判断取值到180，可以使误判率降到接近0
